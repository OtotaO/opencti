...
const defaultColumns: DataTableProps['dataColumns'] = {
  ...
  fromName: {
    ...
    render: ({ from }, { column: { size }, t_i18n }) => {
      const value = from ? defaultValue(from) : t_i18n('Restricted');
      return (<Tooltip title={value}>{truncate(value, size * 0.113)}</Tooltip>);
    },
  },
  incident_type: {
  ...
  name: {
    ...
    flexSize: 25,
    isSortable: true,
    render: (data, { column: { size } }) => (<Tooltip title={defaultValue(data)}>{truncate(defaultValue(data), size * 0.113)}</Tooltip>),
  },
  note_types: {
    label: 'Type',
    flexSize: 10,
  ...
  toName: {
    ...
    render: ({ to }, { column: { size }, t_i18n }) => {
      const value = to ? defaultValue(to) : t_i18n('Restricted');
      return (<Tooltip title={value}>{truncate(value, size * 0.113)}</Tooltip>);
    },
  },
  url: {
