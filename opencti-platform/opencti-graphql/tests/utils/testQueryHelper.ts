...
import { executeInternalQuery, queryAsAdmin, ADMIN_API_TOKEN, API_URI } from './testQuery';
...
export const requestFileFromStorageAsAdmin = async (storageId: string) => {
  ...
  return axios.get(storageGetUrl, { headers: { Authorization: `Bearer ${ADMIN_API_TOKEN}` } })
    .then((res) => {
      return res.data;
    })
    .catch((error) => {
