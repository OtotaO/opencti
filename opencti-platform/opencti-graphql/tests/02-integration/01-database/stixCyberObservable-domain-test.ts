...
describe('Testing Artifact merge with files on S3 (see issue 6258)', () => {
  ...
  it('should files from both artifact be on the final merged artifact', async () => {
    ...
    console.log(`artifact1.id: ${artifact1.stix_id}`);
    console.log(`artifact1.id: ${artifact1.id}`);
    ...
    console.log(`artifact2.id: ${artifact2.stix_id}`);
    console.log(`artifact2.id: ${artifact2.id}`);

    // WHEN merge is called (via taskManager executeMerge)
